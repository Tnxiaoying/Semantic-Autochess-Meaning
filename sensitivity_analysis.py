...
def generate_scenario_simple(n_particles, mode='random'):
    ...
    remainder = n_particles % N_COLORS
    
    colors = []
    for c in range(N_COLORS): 
        colors.extend([c] * particles_per_color)
    
    # Handle remainder: assign extra particles to colors sequentially
    ...
    if mode == 'random':
        ...
        # Calculate base counts per center
        counts_per_center = [particles_per_color] * N_COLORS
        for i in range(remainder):
            counts_per_center[i] += 1
            
        for c_idx in range(N_COLORS):
            ct = 0
            target_count = counts_per_center[c_idx]
    ...
    # Re-verify color list length just in case
    if len(colors) != n_particles:
        # Fallback padding if logic above fails (should not happen with remainder logic)
        while len(colors) < n_particles:
            colors.append(random.randint(0, N_COLORS-1))
...
def run_sensitivity_analysis():
    ...
    ax1.legend(['Energy Saving', 'Critical Region'])
